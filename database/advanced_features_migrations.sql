...
ALTER TABLE tb_tasks ADD COLUMN is_recurring BOOLEAN DEFAULT FALSE;
ALTER TABLE tb_tasks ADD COLUMN recurrence_type ENUM('daily', 'weekly', 'monthly', 'yearly') NULL;
ALTER TABLE tb_tasks ADD COLUMN recurrence_interval INT DEFAULT 1;
ALTER TABLE tb_tasks ADD COLUMN recurrence_end_date DATE NULL;
ALTER TABLE tb_tasks ADD COLUMN parent_recurring_task_id INT NULL;
ALTER TABLE tb_tasks ADD FOREIGN KEY (parent_recurring_task_id) REFERENCES tb_tasks(id) ON DELETE CASCADE;
...
CREATE TABLE tb_task_attachments (
    ...
    FOREIGN KEY (uploaded_by) REFERENCES tb_users(id) ON DELETE CASCADE
);

-- 3. Dependências de Tarefas
CREATE TABLE tb_task_dependencies (
    id INT AUTO_INCREMENT PRIMARY KEY,
    task_id INT NOT NULL,
    depends_on_task_id INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (task_id) REFERENCES tb_tasks(id) ON DELETE CASCADE,
    FOREIGN KEY (depends_on_task_id) REFERENCES tb_tasks(id) ON DELETE CASCADE,
...
CREATE TABLE tb_user_badges (
    ...
    FOREIGN KEY (badge_id) REFERENCES tb_badges(id) ON DELETE CASCADE,
...
INSERT INTO tb_badges (name, description, icon, criteria_type, criteria_value) VALUES
...
ALTER TABLE tb_tasks ADD COLUMN selected_for_bulk BOOLEAN DEFAULT FALSE;

-- Índices para performance
CREATE INDEX idx_tasks_recurring ON tb_tasks(is_recurring);
CREATE INDEX idx_tasks_due_date ON tb_tasks(due_date);
CREATE INDEX idx_user_statistics_date ON tb_user_statistics(date);
CREATE INDEX idx_activity_log_user_date ON tb_activity_log(user_id, created_at);
