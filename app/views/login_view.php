...
<body class="flex items-center justify-center p-4">
    <div class="login-container bg-white rounded-xl shadow-xl w-full max-w-md p-8 md:p-10">
        ...
        <?php
        $showNotification = isset($_GET['message']);
        $messageText = '';
        ...
        <div id="notification" class="fixed top-4 right-4 p-4 rounded shadow-md transition-all duration-300 
        ...
        <?= $notificationType === 'error' ? 'bg-red-100 border-l-4 border-red-500 text-red-700' : '' ?>">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <svg class="h-5 w-5 <?= $notificationType === 'success' ? 'text-green-500' : 'text-red-500' ?>"
                        ...
                        <?php else: ?>
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 8.707a1 1 0 00-1.414-1.414L10 10.586l2.707-2.707a1 1 0 10-1.414-1.414L10 9.172 8.707 8.707z"
                                clip-rule="evenodd" />
                        <?php endif; ?>
                    </svg>
                </div>
                <div class="ml-3">
                    <p class="text-sm"><?= htmlspecialchars($messageText) ?></p>
                </div>
            </div>
        </div>
